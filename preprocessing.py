...
def main():
    ...
    def flatten_main(example):
        return {"source":example[summary_column],"target":example[text_column]}
    ...
    def flatten_nli2(example):
        ...
        return {"source":prefix + example['source'],"target":example['target'],"label":example["label"]}

    nli_columns = nli_dataset["train"].column_names
    nli_dataset = nli_dataset.map(function=flatten_nli2, remove_columns=nli_columns, num_proc=1)

    print(nli_dataset)
    main_dataset.save_to_disk(dataset_dict_path=pathlib.Path(args.out_dataset_dir,"main_dataset"))
    nli_dataset.save_to_disk(dataset_dict_path=pathlib.Path(args.out_dataset_dir,"nli_dataset"))
